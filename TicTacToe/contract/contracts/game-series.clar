...
(define-public (create-series
    ...
    (let (
            ...
            (config (unwrap! (map-get? series-configs games-to-win)
                (err ERR_INVALID_SERIES_LENGTH)
            ))
            (series-data {
...
(define-public (join-series (series-id uint))
    (let (
        ...
        (asserts! (is-eq (get status series-data) SERIES_STATUS_WAITING)
            (err ERR_SERIES_ALREADY_STARTED)
        )
        ...
        (asserts! (not (is-eq contract-caller (get player-one series-data)))
            (err ERR_ALREADY_JOINED)
        )

        ;; Transfer series bet from joiner
        (try! (stx-transfer? (get series-bet series-data) contract-caller
        ...
        (try! (contract-call? .player-registry record-series-participation
...
(define-public (start-next-game (series-id uint))
    (let (
        ...
        (asserts! (is-eq (get status series-data) SERIES_STATUS_ACTIVE)
            (err ERR_SERIES_FINISHED)
        )
        ...
        ;; NOTE: Series games must be created externally via regular tic-tac-toe.create-game
        ;; This avoids circular dependency. The game-id should be linked to the series
        ;; using link-game-to-series function after creation

        ;; Just update series status to show a game is expected
        (map-set game-series series-id
            (merge series-data {
                total-games-played: game-number,
                current-game-id: none, ;; Will be set when game is linked
            })
        )

        ;; Reserve slot for the game
        (map-set series-games {
            series-id: series-id,
            game-number: game-number,
        } {
            game-id: u0, ;; Will be updated when game is linked
            winner: none,
            completed-at: none,
            game-duration: u0,
        })

        ;; Log that series is ready for next game
        (print {
            action: "series-ready-for-game",
            series-id: series-id,
            game-number: game-number,
        })
        (ok game-number)
...
;; #[allow(unchecked_data)]
(define-public (record-game-completion
        (series-id uint)
        (game-id uint)
        (winner principal)
    )
    (let (
            ...
            (game-key {
                series-id: series-id,
                game-number: (+ game-number u1),
            })
            (is-player-one-winner (is-eq winner (get player-one series-data)))
            (new-p1-wins (if is-player-one-winner
                (+ (get player-one-wins series-data) u1)
                (get player-one-wins series-data)
            ))
            (new-p2-wins (if is-player-one-winner
                (get player-two-wins series-data)
                (+ (get player-two-wins series-data) u1)
            ))
        ...
        ;; Only series players can record game completion (removes circular dependency)
        (asserts!
            (or
                (is-eq contract-caller (get player-one series-data))
                (is-eq (some contract-caller) (get player-two series-data))
        ...
        ;; Update game record
        (map-set series-games game-key
            (merge
                (unwrap! (map-get? series-games game-key)
                    (err ERR_SERIES_NOT_FOUND)
                ) {
                winner: (some winner),
        ...
        (map-set game-series series-id
            (merge series-data {
                ...
                status: (if is-series-complete
                    SERIES_STATUS_COMPLETED
                    SERIES_STATUS_ACTIVE
                ),
                completed-at: (if is-series-complete
        ...
        (if is-series-complete
            (begin
                (try! (distribute-series-prizes series-id
                    (unwrap! series-winner (err ERR_SERIES_NOT_FOUND))
                ))
                (try! (contract-call? .player-registry record-series-win
                    (unwrap! series-winner (err ERR_SERIES_NOT_FOUND))
                ))
            )
            true
        ...
        ;; Log game completion
        (print {
            action: "series-game-completed",
            series-id: series-id,
            winner: winner,
            series-complete: is-series-complete,
        })
...
(define-private (distribute-series-prizes
        (series-id uint)
        (winner principal)
    )
    (let (
        ...
        (print {
            action: "series-prizes-distributed",
            series-id: series-id,
            winner: winner,
            amount: winner-prize,
        })
...
(define-read-only (get-series-game
        (series-id uint)
        (game-number uint)
    )
    (map-get? series-games {
        series-id: series-id,
        game-number: game-number,
    })
)
...
(map-set series-configs u2 {
    max-games: u3,
    series-name: "Best of 3",
    min-bet: u1000000,
}) ;; Best of 3
(map-set series-configs u3 {
    max-games: u5,
    series-name: "Best of 5",
    min-bet: u2000000,
}) ;; Best of 5  
(map-set series-configs u4 {
    max-games: u7,
    series-name: "Best of 7",
    min-bet: u5000000,
}) ;; Best of 7
