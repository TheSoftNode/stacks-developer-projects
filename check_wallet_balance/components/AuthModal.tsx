...
interface AuthModalProps {
  ...
  type: 'login' | 'signup';
  onSuccess: () => void;
  onAuthSuccess?: () => void; // New prop to refresh auth status
}
  ...
  const handleWalletAuth = async (authType: 'login' | 'signup') => {
    ...
    try {
      ...
      if (authType === 'signup') {
        ...
        toast.success('Welcome to WalletMonitor! Registration successful.');
      } else {
        // Login with wallet
      ...
      if (result.success) {
        ...
        // Small delay to ensure auth status refreshed
        setTimeout(() => {
            ...
            >
              {isWalletLoading ? (
                ...
                <>
                  ...
                  {type === 'login' ? 'Login with Wallet' : 'Sign up with Wallet'}
                </>
              )}
            </Button>
            
            <div className="text-xs text-muted-foreground text-center">
              Connect your Stacks wallet for secure authentication
            </div>
          </div>
