...
(define-map tournament-brackets
    { tournament-id: uint, round: uint, position: uint }
    {
...
(define-public (create-tournament (tournament-type uint) (max-players uint) (entry-fee uint))
    (let (
        (tournament-id (var-get latest-tournament-id))
        (config (unwrap! (map-get? tournament-configs max-players) (err ERR_INVALID_TOURNAMENT_SIZE)))
        (tournament-data {
    ...
    (asserts! (or (is-eq tournament-type TOURNAMENT_TYPE_SINGLE_ELIMINATION) 
                  (is-eq tournament-type TOURNAMENT_TYPE_DOUBLE_ELIMINATION)) (err ERR_INVALID_TOURNAMENT_SIZE))
    ;; Validate entry fee meets minimum
    (asserts! (>= entry-fee (contract-call? .platform-manager get-min-bet-amount)) (err ERR_UNAUTHORIZED))
    ...
    (map-set tournament-participants { tournament-id: tournament-id, player: contract-caller } {
    ...
    (print { action: "create-tournament", tournament-id: tournament-id, creator: contract-caller, max-players: max-players })
    (ok tournament-id)
    )
)
...
(define-public (join-tournament (tournament-id uint))
    (let (
        (tournament-data (unwrap! (map-get? tournaments tournament-id) (err ERR_TOURNAMENT_NOT_FOUND)))
        (participant-key { tournament-id: tournament-id, player: contract-caller })
        ...
        (updated-tournament (merge tournament-data {
            ...
            prize-pool: (+ (get prize-pool tournament-data) (get entry-fee tournament-data))
        }))
    )
    ;; Check if platform is paused
    (asserts! (not (contract-call? .platform-manager is-paused)) (err ERR_UNAUTHORIZED))
    ;; Ensure tournament exists and is in registration
    (asserts! (is-eq (get status tournament-data) TOURNAMENT_STATUS_REGISTRATION) (err ERR_TOURNAMENT_ALREADY_STARTED))
    ;; Ensure tournament isn't full
    (asserts! (< (get current-players tournament-data) (get max-players tournament-data)) (err ERR_TOURNAMENT_FULL))
    ...
    (try! (stx-transfer? (get entry-fee tournament-data) contract-caller (as-contract tx-sender)))
    ...
    (if (is-eq (get current-players updated-tournament) (get max-players updated-tournament))
        (begin
            (print { action: "tournament-full", tournament-id: tournament-id })
            true
        )
        true
    )
...
(define-public (start-tournament (tournament-id uint))
    (let (
        (tournament-data (unwrap! (map-get? tournaments tournament-id) (err ERR_TOURNAMENT_NOT_FOUND)))
        (config (unwrap! (map-get? tournament-configs (get max-players tournament-data)) (err ERR_INVALID_TOURNAMENT_SIZE)))
    )
    ;; Check authorization (creator or auto-start when full)
    (asserts! (or 
        (is-eq contract-caller (get creator tournament-data))
        (is-eq (get current-players tournament-data) (get max-players tournament-data))
    ) (err ERR_UNAUTHORIZED))
    ;; Ensure tournament is in registration status
    (asserts! (is-eq (get status tournament-data) TOURNAMENT_STATUS_REGISTRATION) (err ERR_TOURNAMENT_ALREADY_STARTED))
    ;; Ensure minimum players
    (asserts! (>= (get current-players tournament-data) (get min-players config)) (err ERR_INSUFFICIENT_PLAYERS))
    ...
    (print { action: "start-tournament", tournament-id: tournament-id, players: (get current-players tournament-data) })
    (ok true)
    )
)
...
(define-private (get-active-players-in-round (tournament-id uint) (round uint))
...
(define-private (create-bracket-match (tournament-id uint) (round uint) (position uint) (player-one (optional principal)) (player-two (optional principal)))
    (let (
        (bracket-key { tournament-id: tournament-id, round: round, position: position })
    )
...
(define-public (generate-first-round-brackets (tournament-id uint))
    (let (
        (tournament-data (unwrap! (map-get? tournaments tournament-id) (err ERR_TOURNAMENT_NOT_FOUND)))
    )
    ;; Verify tournament is active and in first round
    (asserts! (is-eq (get status tournament-data) TOURNAMENT_STATUS_ACTIVE) (err ERR_TOURNAMENT_NOT_STARTED))
    (asserts! (is-eq (get current-round tournament-data) u1) (err ERR_INVALID_BRACKET))
    
    ;; For now, create empty bracket slots (will be populated off-chain or by separate function)
    ;; This is the minimal on-chain bracket generation
    (print { action: "generate-brackets", tournament-id: tournament-id, round: u1 })
    (ok true)
    )
)
...
;; #[allow(unchecked_data)]
(define-public (record-match-result (tournament-id uint) (round uint) (position uint) (winner principal) (game-id uint))
    (let (
        (tournament-data (unwrap! (map-get? tournaments tournament-id) (err ERR_TOURNAMENT_NOT_FOUND)))
        (bracket-key { tournament-id: tournament-id, round: round, position: position })
        (bracket-data (unwrap! (map-get? tournament-brackets bracket-key) (err ERR_INVALID_BRACKET)))
    )
    ...
    (map-set tournament-brackets bracket-key (merge bracket-data {
        winner: (some winner),
    ...
    ;; Log match result
    (print { action: "match-result", tournament-id: tournament-id, round: round, position: position, winner: winner })
    (ok true)
    )
)
...
(define-public (advance-tournament-round (tournament-id uint))
    ...
    (asserts! (or 
        (is-eq contract-caller (get creator tournament-data))
        (contract-call? .platform-manager is-authorized-contract contract-caller)
    ) (err ERR_UNAUTHORIZED))
    ;; Verify tournament is active
    (asserts! (is-eq (get status tournament-data) TOURNAMENT_STATUS_ACTIVE) (err ERR_TOURNAMENT_NOT_STARTED))
    ...
    (if (is-eq current-round total-rounds)
        ...
        (begin
            ...
            (print { action: "tournament-advanced", tournament-id: tournament-id, new-round: (+ current-round u1) })
            (ok true)
        )
    )
    )
...
;; #[allow(unchecked_data)]
(define-public (distribute-tournament-prizes (tournament-id uint) (winner principal))
    (let (
        (tournament-data (unwrap! (map-get? tournaments tournament-id) (err ERR_TOURNAMENT_NOT_FOUND)))
        (config (unwrap! (map-get? tournament-configs (get max-players tournament-data)) (err ERR_INVALID_TOURNAMENT_SIZE)))
    ...
    (asserts! (is-eq (get status tournament-data) TOURNAMENT_STATUS_COMPLETED) (err ERR_TOURNAMENT_NOT_STARTED))
    ;; Verify winner is valid
    (asserts! (is-some (get winner tournament-data)) (err ERR_TOURNAMENT_NOT_FOUND))
    ;; Verify winner matches the provided winner
    (asserts! (is-eq (some winner) (get winner tournament-data)) (err ERR_UNAUTHORIZED))
    
    ;; Calculate and distribute prizes
    (let (
        (winner-percentage (unwrap! (element-at? prize-distribution u0) (err ERR_INVALID_TOURNAMENT_SIZE)))
    ...
    (print { action: "prizes-distributed", tournament-id: tournament-id, winner: winner, amount: net-winner-prize })
    (ok net-winner-prize)
    )
    )
)
...
(define-public (cancel-tournament (tournament-id uint))
    ...
    (asserts! (not (is-eq (get status tournament-data) TOURNAMENT_STATUS_COMPLETED)) (err ERR_TOURNAMENT_ALREADY_STARTED))
    ...
    ;; Log cancellation
    (print { action: "tournament-cancelled", tournament-id: tournament-id, refund-pool: total-refund })
    (ok true)
    )
)
...
(define-public (claim-tournament-refund (tournament-id uint))
    (let (
        (tournament-data (unwrap! (map-get? tournaments tournament-id) (err ERR_TOURNAMENT_NOT_FOUND)))
        (participant-key { tournament-id: tournament-id, player: contract-caller })
        (participant-data (unwrap! (map-get? tournament-participants participant-key) (err ERR_NOT_REGISTERED)))
        (entry-fee (get entry-fee tournament-data))
    )
    ;; Verify tournament is cancelled
    (asserts! (is-eq (get status tournament-data) TOURNAMENT_STATUS_CANCELLED) (err ERR_TOURNAMENT_NOT_STARTED))
    ...
    (map-set tournament-participants participant-key (merge participant-data {
        is-active: false,
    ...
    ;; Log refund
    (print { action: "refund-claimed", tournament-id: tournament-id, player: contract-caller, amount: entry-fee })
    (ok entry-fee)
    )
)
...
(define-read-only (get-tournament-bracket (tournament-id uint) (round uint) (position uint))
    (map-get? tournament-brackets { tournament-id: tournament-id, round: round, position: position })
)

;; Initialize tournament configurations
(map-set tournament-configs u4 { min-players: u4, total-rounds: u2, prize-distribution: (list u7000 u3000) }) ;; 4 players: 70% winner, 30% runner-up
(map-set tournament-configs u8 { min-players: u6, total-rounds: u3, prize-distribution: (list u5000 u3000 u1500 u500) }) ;; 8 players
(map-set tournament-configs u16 { min-players: u8, total-rounds: u4, prize-distribution: (list u4000 u2500 u1500 u1000 u500 u500) }) ;; 16 players
(map-set tournament-configs u32 { min-players: u16, total-rounds: u5, prize-distribution: (list u3500 u2000 u1500 u1000 u800 u600 u400 u200) }) ;; 32 players
