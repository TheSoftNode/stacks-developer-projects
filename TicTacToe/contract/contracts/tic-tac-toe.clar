...
(define-private (is-game-timed-out (game-data {
    player-one: principal,
    player-two: (optional principal),
    is-player-one-turn: bool,
    bet-amount: uint,
    ...
    tournament-position: (optional uint),
}))
    (let ((timeout-blocks (contract-call? .platform-manager get-move-timeout)))
        (and
            (is-eq (get status game-data) "active")
            (> (- stacks-block-height (get last-move-block game-data))
                timeout-blocks
            )
...
(define-public (create-game
    ...
    (let (
        ...
        (asserts! (not (contract-call? .platform-manager is-paused))
            (err ERR_PLATFORM_PAUSED)
        )
        ;; Ensure that user has put up a bet amount greater than the minimum from platform manager
        (asserts!
            (>= bet-amount (contract-call? .platform-manager get-min-bet-amount))
            (err ERR_INVALID_BET_AMOUNT)
        )
...
(define-public (create-tournament-game
    ...
    (let (
        ...
        (asserts! (is-eq contract-caller .tournament-manager)
...
(define-public (play
    ...
    (let (
            ...
            (new-status (if (or is-now-winner is-now-draw)
                "finished"
                "active"
            ))
        ...
        (if is-now-winner
            (let (
                    ...
                    (loser (if is-player-one-turn
                        (unwrap-panic (get player-two original-game-data))
                        (get player-one original-game-data)
                    ))
                ...
                (try! (contract-call? .player-registry record-game-result winner loser
                    prize-amount (get bet-amount game-data)
                ))
            )
            ;; If it's a draw, return bet to both players
            (if is-now-draw
                (begin
                    (try! (as-contract (stx-transfer? (get bet-amount game-data) tx-sender
                        (get player-one original-game-data)
                    )))
                    (try! (as-contract (stx-transfer? (get bet-amount game-data) tx-sender
                        (unwrap-panic (get player-two original-game-data))
                    ...
                    (try! (contract-call? .player-registry record-draw-result
                        (get player-one original-game-data)
                        (unwrap-panic (get player-two original-game-data))
                    ))
                )
        ...
        ;; Handle tournament game completion if applicable (always succeeds)
        (unwrap-panic (handle-tournament-game-completion game-id game-data))
...
;; NOTE: Tournament manager will query game state instead of being notified
;; This avoids circular dependency between tic-tac-toe and tournament-manager
(define-private (handle-tournament-game-completion
        (game-id uint)
        (game-data {
            player-one: principal,
            player-two: (optional principal),
    ...
    ;; Log tournament game completion event for off-chain indexing
    ;; Tournament manager can query game state using get-game function
    (begin
        (if (is-some (get tournament-id game-data))
            (begin
                (print {
                    action: "tournament-game-completed",
                    game-id: game-id,
                    tournament-id: (get tournament-id game-data),
                    tournament-round: (get tournament-round game-data),
                    tournament-position: (get tournament-position game-data),
